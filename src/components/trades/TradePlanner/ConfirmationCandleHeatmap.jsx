// src/components/trades/TradePlanner/ConfirmationCandleHeatmap.jsx
...
const ConfirmationCandleHeatmap = ({ data, session, isLoading, onCandleSelect, selectedCandle }) => {
  // Get confirmation times based on session
  ...
  // Calculate the best confirmation time
  const bestTime = data.bestConfirmationTime;
  const bestTimeData = data.candleMap[bestTime];
  ...
  return (
    <Paper sx={{ p: 2, height: '100%' }}>
      ...
      {/* Best confirmation time */}
      {bestTime && (
        <Box sx={{ mb: 2 }}>
          <Typography variant="subtitle1" fontWeight="bold">
            Best Confirmation Time: {bestTime}
          </Typography>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
        {getConfirmationTimes().map((time) => {
          const timeData = data.candleMap[time] || { trades: [], totalR: 0, winrate: 0 };
          const color = getColorForR(timeData.totalR, timeData.trades.length);
          const isBestTime = time === bestTime;
          const isSelected = time === selectedCandle;
            ...
            >
              <Box
                sx={{
                  ...
                  alignItems: 'center',
                  border: isSelected ? '3px solid black' : (isBestTime ? '2px solid black' : '1px solid #ccc'),
                  borderRadius: 1,
                  cursor: 'pointer',
                }}
                onClick={() => timeData.trades.length > 0 && onCandleSelect(time)}
              >
      ...
      <Box sx={{ mt: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Box sx={{ width: 20, height: 20, border: '3px solid black', mr: 1 }} />
...
export default ConfirmationCandleHeatmap;